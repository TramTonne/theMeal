export default async function handler(req, res) {
  ...
  const regenCalories = targetCalories - sumCalories;
  const regenProtein  = dailyMacros.protein - sumProtein;
  const regenFat      = dailyMacros.fat - sumFat;
  const regenCarbs    = dailyMacros.carbs - sumCarbs;

  // Build prompt for regenerating the specific meal
  const prompt = `
...
Do not include any extra text, numbering outside these lists, or explanations—just these seven sections.
  ...
  function parseSingleMeal(text) {
    ...
    lines.forEach(line => {
      if (line.startsWith('Name:')) {
        ...
        if (mode === 'recipe') recipe.push(item);
      }
    });
  ...
  // Return the updated meal plan into parts for the front end
  res.status(200).json({
    targetCalories,
    dailyMacros,
